...
model_path = project_path + "ecg_cnn_model.pt"
...
'''
train_epochs函数是整个模型训练的控制中心，
它在每个训练周期(epoch)中依次调用train_steps和test_steps函数来完成训练和验证过程。
train_epochs函数还负责记录和输出每个周期的训练和验证结果，并将它们写入tensorboard的log文件中以便于后续分析。
'''
...
def main():
    config = {
        'seed': 42,         # 随机数种子
        'test_ratio': 0.3,  # 测试集在数据集中所占的比例
        'num_epochs': 20,   # 模型训练的总轮数
